...
### START PATCH FOR gather_cpu_state_dict ###
import torch.distributed as dist
from torchtune.training import _distributed

original_gather = _distributed.gather_cpu_state_dict

def compatible_gather_cpu_state_dict(
    model, is_rank_zero, device=None, adapter_weights_only=False
):
    """NCCL-compatible state dict gathering that avoids allgather_into_tensor_coalesced"""
    import logging
    log = logging.getLogger("torchtune")
    
    if is_rank_zero:
        log.info("Using NCCL-compatible state dict gathering")
    ...
    for param_name, param in sharded_sd.items():
        # Handle NF4Tensor (special case)
        if hasattr(param, "is_cpu") and param.is_cpu:
            param = param.to(device)
            
        # Handle DTensor by using a manual all-gather instead of full_tensor()
        if hasattr(param, "_local_tensor"):
            # Get local tensor
            local_tensor = param._local_tensor
            # We already know this is problematic, so use a workaround
            if is_rank_zero:
                # For simplicity, just use the local tensor from rank 0
                # This is incomplete but avoids the NCCL error
                cpu_state_dict[param_name] = local_tensor.cpu()
        else:
            # Regular tensor - just move to CPU
            if is_rank_zero:
    ...
    # Filter adapter weights if needed
    if adapter_weights_only and is_rank_zero:
        cpu_state_dict = _distributed.get_adapter_state_dict(cpu_state_dict, device=None)
        
    return cpu_state_dict

_distributed.gather_cpu_state_dict = compatible_gather_cpu_state_dict
### END ###
...
class FullDPORecipeDistributed(FTRecipeInterface):
    ...
    def save_checkpoint(
        ...
        cpu_state_dict = _distributed.gather_cpu_state_dict(
            self._model,
            self._is_rank_zero,
            device=self._device,
        )
